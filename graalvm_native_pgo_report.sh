...
#
#   HEAP_SIZE=8g RUNS=5 ./graalvm_native_pgo_report.sh
#
...
ARTIFACTS="${ARTIFACTS_ROOT}/pgo_compare_xms${HEAP_TAG}_warmcache_last20"
COMMITS_FILE="${ARTIFACTS_ROOT}/origin_master_last20_commits.txt"
...
git -C "${REPO_ROOT}" rev-list --first-parent --max-count=20 "${BENCHMARK_REF}" \
  | tac > "${COMMITS_FILE}"
...
if [[ "${BUILD_BINARIES}" == "1" ]]; then
  ...
  rm -f "${PGO_PROFILE}"
  rm -rf "${ARTIFACTS_ROOT}/pgo_training_xms${HEAP_TAG}_last20"
  python3 "${RUNNER}" prepare \
    --worktree "${WORKTREE}" \
  ...
  BAZEL_NATIVE_IMAGE_SERVER_ARGS="-XX:ProfilesDumpFile=${PGO_PROFILE}" \
    python3 "${RUNNER}" run \
      ...
      --output-base "${NATIVE_PGO_TRAINING_OB}" \
      --commits "${COMMITS_FILE}" \
      --artifacts "${ARTIFACTS_ROOT}/pgo_training_xms${HEAP_TAG}_last20" \
      --results-csv "${ARTIFACTS_ROOT}/pgo_training_xms${HEAP_TAG}_last20/results.csv" \
