...
def _impl(rctx):
    ...
    # Watch workspace entries so changes invalidate this repo without tracking ignored dirs.
    ignore_names = {
        ".git": True,
        ".devcontainer": True,
        ".ijwb": True,
        ...
        "output": True,
        "derived": True,
        "compact_exec_log.binpb.zst": True,
    }
...
genrule(
  ...
  cmd = " | ".join([
    "cat $<",
    "grep -Ev '^(\\\\.git|\\\\.devcontainer|.ijwb|out/|output/|bazel-|derived|tools/defaults/BUILD|compact_exec_log\\\\.binpb\\\\.zst$$)'",
    "grep -Ev '%s'",
    "sort -u > $@",
  ]),
)
